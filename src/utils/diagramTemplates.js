...
2. 关系单独一行，可用：A -> B、A --> B、A *-- B、A o-- B、A <|-- B、A ..> B
3. 属性尽量写成 字段名:类型，方法写成 动词()
4. 优先输出 4-10 个核心类，不要过度展开
...
2. 消息格式：A->B: 内容 或 A-->B: 返回内容
3. 有条件分支时使用 alt 条件 / else 条件 / end
4. 按 用户 -> 前端 -> 后端 -> 数据库 的顺序组织参与者
5. 仅输出 DSL 内容，不要解释，不要加 Markdown 代码块`,
...
1. 主格式：节点A -> 节点B
2. 分支格式：节点A ->|条件| 节点B
3. 标准图元：开始/结束 或 (开始)/(结束)，判断用 {条件?}，输入输出用 /节点/，数据库用 [(数据库)]，子流程用 [[子流程]]
4. 流程应尽量清晰，优先使用 6-12 个节点表达
5. 仅输出 DSL 内容，不要解释，不要加 Markdown 代码块`,
