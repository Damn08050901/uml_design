...
1. 主格式：节点A -> 节点B
2. 分支格式：节点A ->|条件| 节点B
3. 图元：开始/结束（必须显式写“开始”“结束”）、{条件?} 判断菱形、===名称=== 分叉/汇合同步条。其他图元：/输入输出/ 平行四边形、[(数据库)] 数据存储、[[子流程]] 子流程、((连接符))
4. 所有判断节点必须使用花括号包裹，并以 ? 或 ？ 结尾，例如 {信息是否完整？}
5. 流程节点名称尽量简短，优先使用 4-10 个字，不要把一句话塞进节点里
6. 默认主流程保持自上而下垂直展开；判断节点的正向/成功/是 分支优先放在右侧，负向/失败/否 分支优先放在左侧
7. 异常、重复、失败、驳回、提示类分支如果语义上表示流程终止，应直接连接到 结束，不要错误地回到主流程
8. 如果分支需要重新汇入主流程，请在真正的汇合步骤汇入，不要为了省节点把提示节点直接接回主干中段
9. 流程应尽量清晰，优先使用 6-12 个节点表达
10. 仅输出 DSL 内容，不要解释，不要加 Markdown 代码块`,
